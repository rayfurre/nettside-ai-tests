...
async function saveEditorChanges(page: Page): Promise<boolean> {
  try {
    ...
    if (await btn.isDisabled().catch(() => true)) return true; // disabled = no changes = OK
    await btn.click();
...
async function openImageModal(page: Page, skipCount: number = 0): Promise<boolean> {
  try {
    ...
    // Finn klikkbare bilder (synlig, store nok)
    ...
    // Fallback: hvis skipCount >= antall bilder, bruk #0
    const idx = skipCount < clickable.length ? skipCount : 0;

    // Prøv target-bildet først, deretter alle andre som fallback
...
test.describe('Nettside.ai - Komplett test', () => {
  ...
  test('Registrering, generering, kladd og editor', async ({ page, context }) => {
    ...
    // ========================================
    // STEG 7: Hent kladd-URL (dialog med polling opptil 90s)
    // ========================================
    stegStart = Date.now();
    try {
