...
// VERSION: 7.1 (filtrerer draft-404, betalingstest steg 13)
// ===================================================
...
const IGNORED_URL_PATTERNS = [
  ...
  'draft.kundesider.pages.dev',
  'draft--vibe-kundesider.netlify.app',
];
...
test.describe('Nettside.ai - Komplett test', () => {
  ...
  test('Registrering, generering, kladd, editor og betaling', async ({ page, context }) => {
    ...
    if (stillEditing) {
      // Prøv å lagre og gå ut av redigeringsmodus
    ...
    stegStart = Date.now();
    try {
      // Vent på at Publiser-knappen er synlig (kan ta litt etter editor-lukking)
    ...
    stegStart = Date.now();
    try {
      // Pristabellen er en dialog med "Velg plan for publisering"
      ...
      // Verifiser at Pro-planen vises med "Velg Pro"-knapp
    ...
    try {
      ...
      // Vent på at Stripe-siden laster
      ...
      // Verifiser at det er en Stripe checkout-side
      ...
      stegStart = Date.now();
      try {
        // Sjekk at siden inneholder betalingsinformasjon
        await stripePage.waitForLoadState('networkidle', { timeout: 30000 }).catch(() => {});

        // Sjekk etter "Abonner"-knapp eller "Subscribe" (norsk/engelsk)
        ...
        // Lukk Stripe-fanen uten å fullføre betaling
